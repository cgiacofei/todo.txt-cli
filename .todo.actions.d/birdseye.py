...
def printTaskGroup(p, pctage, star):
	if pctage > -1:
		...
		if pctage > 9:	
			...
			displayTotal = "  %d%%"% (pctage, );
		print "%s %s [%s] %s %s %s(%d todo's)"% (star, displayTotal, progressBar, bcolors.PROJECT , p[0], bcolors.ENDC, p[1],)
	else:
                if p[0][0:1] == '@':
                        print "%s %s %s %s (%d todo's)"% (star, bcolors.CONTEXT, p[0], bcolors.ENDC, p[1],)
                else:
                        print "%s %s %s %s (%d todo's)"% (star, bcolors.PROJECT, p[0], bcolors.ENDC, p[1],)
def separator(c):
	sep = ""
	sep = c * 42
	print sep
